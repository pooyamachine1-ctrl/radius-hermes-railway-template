...
# The hosting platform supplies PORT; /data is the persistent volume.
ENV PORT=3000 \
  WEBHOOK_PORT=3000 \
...
RUN mkdir -p /data/workspace /data/.hermes /app/scripts
...
# VibeNest routes external traffic to PORT. The entrypoint starts the agent server on that port.
EXPOSE 3000
