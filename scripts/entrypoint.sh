...
# VibeNest / generic container runtime
# /data is the persistent volume. PORT is supplied by the hosting platform.
...
start_hermes_log_forwarders() {
  ...
  echo "[bootstrap] Forwarding Hermes log files from ${logs_dir} to container stdout/stderr..."
...
# === Public URL: VibeNest uses A2A_PUBLIC_URL / PUBLIC_URL.
# Keep Railway fallback only for backwards compatibility.
if [[ -z "${A2A_PUBLIC_URL:-}" ]]; then
  if [[ -n "${PUBLIC_URL:-}" ]]; then
    export A2A_PUBLIC_URL="${PUBLIC_URL}"
  elif [[ -n "${RAILWAY_PUBLIC_DOMAIN:-}" ]]; then
    export A2A_PUBLIC_URL="https://${RAILWAY_PUBLIC_DOMAIN}"
  fi
fi

if [[ -z "${A2A_PUBLIC_URL:-}" ]]; then
  echo "[bootstrap] WARNING: A2A_PUBLIC_URL is not set. Set it to the public HTTPS URL of this deployment if A2A/discovery is required." >&2
else
  echo "[bootstrap] Public agent URL: ${A2A_PUBLIC_URL}"
fi

echo "[bootstrap] Runtime port: ${PORT}; webhook port: ${WEBHOOK_PORT}"
...
for key in \
  ...
  PORT WEBHOOK_PORT WEBHOOK_SECRET DEBUG_SKILLS \
...
echo "[bootstrap] Starting agent server on 0.0.0.0:${PORT}..."
